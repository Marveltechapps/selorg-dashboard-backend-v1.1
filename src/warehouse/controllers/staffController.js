...
const getStaffSummary = async (req, res, next) => {
  try {
    ...
    // workforceService doesn't have getStaffSummary, using listStaff instead
...
const listStaff = async (req, res, next) => {
  try {
    ...
    const result = await workforceService.listStaff(filters);
...
const listShifts = async (req, res, next) => {
  try {
    ...
    const result = await workforceService.listShifts(filters);
    res.status(200).json(result);
  } catch (error) {
    next(error);
  }
