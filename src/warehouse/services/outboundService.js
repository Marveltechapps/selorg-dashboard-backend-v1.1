...
const outboundService = {
  ...
  createBatch: async (batchData = {}) => {
    ...
    if (batchData.status === 'preparing') batchData.status = 'pending';
    const batch = await PickingBatch.create(batchData);
    return {
      id: batch.id,
      batchId: batch.id,
      orderCount: batch.orders ? batch.orders.length : 0,
      totalItems: batch.itemCount || 0,
      picker: batch.pickerId || 'Unassigned',
      status: batch.status === 'pending' ? 'preparing' : batch.status === 'in-progress' ? 'picking' : batch.status,
      progress: batch.status === 'completed' ? 100 : batch.status === 'in-progress' ? 50 : 0
    };
  },
